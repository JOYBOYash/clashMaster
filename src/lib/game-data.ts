...
export const getMaxLevelForItem = (itemName: string, thLevel: number): number => {
    ...
    // The levels array is 1-indexed for TH levels, so we access it with thLevel - 1
...
export const getBuildingCountsForTownHall = (thLevel: number): Record<string, number> => {
    const counts: Record<string, {th: number, count: number}[]> = {
        'Cannon': [{th:1, count:2}, {th:3, count:3}, {th:6, count:4}, {th:7, count:5}, {th:10, count:6}, {th:12, count:7}, {th:15, count: 8}],
        'Archer Tower': [{th:2, count:1}, {th:4, count:2}, {th:5, count:3}, {th:7, count:4}, {th:8, count:5}, {th:9, count:6}, {th:10, count:7}, {th:12, count:8}, {th:15, count:9}],
        'Mortar': [{th:3, count:1}, {th:6, count:2}, {th:8, count:3}, {th:9, count:4}],
        'Air Defense': [{th:4, count:1}, {th:6, count:2}, {th:7, count:3}, {th:9, count:4}],
        'Wizard Tower': [{th:5, count:1}, {th:6, count:2}, {th:8, count:3}, {th:9, count:4}, {th:11, count:5}, {th:14, count:6}],
        'Air Sweeper': [{th:6, count:1}, {th:9, count:2}],
        'Hidden Tesla': [{th:7, count:2}, {th:8, count:3}, {th:9, count:4}, {th:12, count:5}, {th:15, count:6}],
        'Bomb Tower': [{th:8, count:1}, {th:10, count:2}],
        'X Bow': [{th:9, count:2}, {th:10, count:3}, {th:11, count:4}],
        'Inferno Tower': [{th:10, count:2}, {th:14, count:3}],
        'Eagle Artillery': [{th:11, count:1}],
        'Scattershot': [{th:13, count:2}],
        'Spell Tower': [{th:15, count:2}],
        'Monolith': [{th:15, count:1}],
        'Army Camp': [{th:1, count:1}, {th:3, count:2}, {th:4, count:3}, {th:5, count:4}],
        'Gold Storage': [{th:1, count:1}, {th:3, count:2}, {th:7, count:3}, {th:9, count:4}],
        'Elixir Storage': [{th:1, count:1}, {th:3, count:2}, {th:7, count:3}, {th:9, count:4}],
        'Dark Elixir Drill': [{th:8, count:1}, {th:9, count:2}, {th:10, count:3}, {th:13, count:4}],
        'Wall': [{th:2, count:25}, {th:3, count:50}, {th:4, count:75}, {th:5, count:100}, {th:6, count:125}, {th:7, count:175}, {th:8, count:225}, {th:9, count:250}, {th:10, count:275}, {th:11, count:300}, {th:12, count:300}, {th:13, count:300}, {th:14, count:325}, {th:15, count:325}, {th:16, count:350}],
    };

    const finalCounts: Record<string, number> = {};
    for (const building in counts) {
        let currentCount = 0;
        const tiers = counts[building].sort((a,b) => a.th - b.th);
